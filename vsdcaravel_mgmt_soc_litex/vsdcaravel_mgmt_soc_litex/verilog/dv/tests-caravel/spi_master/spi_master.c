...
void spi_write(uint8_t c)
{
    ...
    reg_spimaster_control = 0x0801;   // 8 bits + start
    spi_wait_done();
}
...
//    reg_spimaster_control = 0x0800;
    ...
    while (reg_spimaster_status != 1);       // wait done
}*/

uint8_t spi_read(void)
{
    spi_write(0x00);                  // clocks 8 cycles
    return (uint8_t)reg_spimaster_rdata;
}
...
/*    reg_mprj_io_34  = GPIO_MODE_MGMT_STD_INPUT_NOPULL;	// SDI
    ...
    reg_mprj_io_34  = GPIO_MODE_MGMT_STD_INPUT_NOPULL; // MISO (from flash)
    reg_mprj_io_35  = GPIO_MODE_MGMT_STD_OUTPUT;       // MOSI (to flash)
    ...
    reg_spimaster_clk_divider = 32;   // added for sim
